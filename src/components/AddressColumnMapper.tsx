...
export function AddressColumnMapper({ availableColumns, onMappingChange, sampleRows = [] }: AddressColumnMapperProps) {
  ...
  const addMapping = () => {
    const newMapping: ColumnMapping = {
      ...
      col1: availableColumns[0] || '',
      col2: availableColumns[1] || '',
      col3: availableColumns[2] || '',
      separator: ', ',
    };
  ...
  const updateMapping = (id: string, patch: Partial<ColumnMapping>) => {
    const updated = mappings.map(m => m.id === id ? { ...m, ...patch } : m);
    setMappings(updated);
    onMappingChange(updated);
  };
  ...
  const previewSample = (mapping: ColumnMapping) => {
    if (sampleRows.length === 0) return 'Jl. Sudirman No.1, Jakarta Pusat, DKI Jakarta';
    return buildAddress(mapping, sampleRows[0]) || '(kolom kosong)';
  };
  ...
  const ColSelect = ({ value, onChange }: { value: string; onChange: (v: string) => void }) => (
    <select
      ...
      className="flex-1 px-2.5 py-1.5 text-xs rounded-lg border bg-transparent outline-none"
      ...
      <div className="p-5 border-b border-border">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {mappings.length > 0 && sampleRows.length > 0 && (
        ...
        <p className="text-xs mt-2" style={{ color: 'hsl(var(--muted-foreground))' }}>
          Gabungkan 2–3 kolom alamat dari database menjadi 1 string alamat lengkap untuk proses geocoding.
          {availableColumns.length === 0 && (
            <span style={{ color: 'hsl(38 92% 55%)' }}> Upload data sistem terlebih dahulu untuk melihat kolom yang tersedia.</span>
          )}
        </p>
      </div>
          ...
          mappings.map((mapping, idx) => (
            <div key={mapping.id} className="rounded-xl border p-4 space-y-3"
              ...
              <div className="flex flex-wrap items-center gap-2">
                <ColSelect value={mapping.col1} onChange={v => updateMapping(mapping.id, { col1: v })} />
                <Plus className="w-3.5 h-3.5 flex-shrink-0" style={{ color: 'hsl(var(--muted-foreground))' }} />
                <ColSelect value={mapping.col2} onChange={v => updateMapping(mapping.id, { col2: v })} />
                <Plus className="w-3.5 h-3.5 flex-shrink-0" style={{ color: 'hsl(var(--muted-foreground))' }} />
                <ColSelect value={mapping.col3} onChange={v => updateMapping(mapping.id, { col3: v })} />
              </div>
                      ...
                      <tbody>
                        {sampleRows.slice(0, 5).map((row, i) => (
                          <tr key={i} style={{ borderTop: '1px solid hsl(var(--border))' }}>
                            ...
                            <td className="px-3 py-1.5 font-mono" style={{ color: 'hsl(var(--primary))' }}>
